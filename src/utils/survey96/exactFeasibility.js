...
export const findSurvey96ExactFeasibility = async ({
  vendorDetails,
  vendorGroupId,
}) => {
  try {
    console.log(
      ">>>>>>>>>>> the value of the VENDOR GROUP ID in findSurvey96ExactFeasibility is : ",
      vendorGroupId,
    );
    ...
    const getGroupFeasibilityResponse = await axios.get(
      `${base_url}/group/${vendorGroupId}/feasibility`,
      {
        headers: {
    ...
    console.log(
      ">>>>>. the value of the GET GROUP FEASIBILITY response in findSurvey96ExactFeasibility is : ",
